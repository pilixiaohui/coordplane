...
Owner批准的预算重基线不改变本文件的对象、状态、命令或不变量。全库production/tests/infra/total envelope固定为`20,000/21,000/22,400`、`21,000/22,000/23,600`、`250/400/600`和`41,250/43,400/46,600`；统计仍按`acceptance.md`的非空、非纯注释物理行口径。Core模块实际SLOC、质量blocker和diff必须进入clean候选revision的LOC JSON，并通过真实多Agent可靠性场景验证状态收敛；重基线reserve不得授权删除本文件合同。

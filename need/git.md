...
Owner批准的预算重基线不改变本文件的private clone、actual HEAD capture、task ref、expected-old CAS、integration、recovery或GC合同。全库production/tests/infra/total envelope固定为`20,000/21,000/22,000`、`21,000/22,000/23,300`、`250/400/600`和`41,250/43,400/45,900`；统计仍按`acceptance.md`的非空、非纯注释物理行口径。Git模块实际SLOC、质量blocker和diff必须进入clean候选revision的LOC JSON，并在真实多Agent场景中验证所有接受结果的lineage、task ref、integration和canonical CAS；不得用重基线reserve删除真实Git crash/fence证据。

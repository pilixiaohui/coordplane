...
func TestProviderAuditFailsClosedWhenTranscriptIdentityIsMissing(t *testing.T) {
	...
	for _, statement := range []string{
		...
		`INSERT INTO cli_sessions (id, attempt_id, runtime_id, agent_id, cli_backend, profile_name, session_native_id, state, start_reason, provider_audit_required, started_at, updated_at) VALUES ('cli_provider_identity', 'att_provider_identity', 'rt_provider_identity', 'developer', 'claude', 'claude', 'native_provider_identity', 'running', 'start', 1, '` + now + `', '` + now + `')`,
	} {

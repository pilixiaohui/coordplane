...
func (c *runtimeController) newMonitor(
	run core.Run,
	ref containerruntime.RuntimeRef,
	entry adapter.CLI,
	control *runControl,
	...
	monitor := &runMonitor{
		...
		waitCancel: cancel, wait: make(chan waitResult, 1), logs: make(chan error, 1),
	}
	go func() {
...
func (c *runtimeController) streamLogs(
	ctx context.Context,
	run core.Run,
	ref containerruntime.RuntimeRef,
	entry adapter.CLI,
		...
		if parseErr == nil {
			...
			case adapter.EventSessionStarted:
				if _, err := c.service.RecordRunSession(context.Background(), core.RunSessionInput{
					RunRuntimeFactInput: runtimeFactInput(run, ref, "session"),
